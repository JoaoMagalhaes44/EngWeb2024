import json

# Carregar o arquivo JSON
with open('mapa-virtual.json', 'r') as f:
    bd = json.load(f)

# Iterar sobre as cidades
for cidade in bd['cidades']:
    # Abrir o arquivo para escrita no modo 'w'
    with open(f'cidades/{str(cidade["id"])}.html', 'w') as f:
        pagHTML = f"""
        <!DOCTYPE html> 
        <html lang="pt-br">

        <head>
            <meta charset="utf-8">
            <meta name="viewport" content="width=device-width, initial-scale=1">
            <title>Cidade: {cidade['nome']}</title>
            <link rel="stylesheet" href="https://www.w3schools.com/w3css/4/w3.css">
            <style>
                body {{
                    font-family: Arial, sans-serif;
                }}
                .w3-card-4 {{
                    max-width: 600px;
                    margin: 50px auto;
                    padding: 20px;
                    box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);
                }}
                .w3-container-header, .w3-container-footer {{
                    background-color: #607D8B;
                    color: #fff;
                    text-align: center;
                    padding: 10px;
                }}
                .w3-container-content {{
                    padding: 20px;
                }}
            </style>
        </head>

        <body>

            <div class="w3-card-4">

                <header class="w3-container w3-container-header">
                    <h2>Cidade: {cidade['nome']}</h2>
                </header>

                <div class="w3-container w3-container-content">
                    <table class="w3-table w3-striped">
                        <tr>
                            <td><b>Id</b></td>
                            <td>{cidade['id']}</td>
                        </tr>
                        <tr>
                            <td><b>Distrito</b></td>
                            <td>{cidade['distrito']}</td>
                        </tr>
                        <tr>
                            <td><b>População</b></td>
                            <td>{cidade['população']}</td>
                        </tr>
                        <tr>
                            <td><b>Descrição</b></td>
                            <td>{cidade['descrição']}</td>
                        </tr>
                    </table>

                    <header class="w3-container w3-container-header">
                        <h4>Ligações: <!-- OUTRAS_CIDADES --></h4>
                    </header>
                </div>

                <footer class="w3-container w3-container-footer">
                    <p>Generated by CidadeApp::EngWeb2024::A100740</p>
                    <p>
                        <a href="http://localhost:7777/">Voltar à página principal</a>
                    </p>
                </footer>

            </div>

        </body>

        </html>
        """
        # Escrever o conteúdo no arquivo
        f.write(pagHTML)
